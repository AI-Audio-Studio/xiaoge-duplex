# Xiaoge Duplex Speech

> 基于 LiveKit，研发一个**更懂你的全双工语音交互引擎**。

**小歌（Xiaoge Duplex Speech）** 是一个全双工中文语音交互引擎：用户说话与引擎应答可同时进行，
支持随时打断、即时反应，让对话更自然、更"懂你"。技术栈：**Qwen LLM**（OpenAI 兼容网关）+
**FunASR / Qwen3 流式 STT** + **百炼(DashScope) TTS** + **sherpa-onnx 关键词强打断 KWS**，
含浏览器测试面板。基于 LiveKit Agents 框架二次开发，入口为
...
> ⚠️ 团队协作须知：
> - **不要把真实密钥提交进库**。`.env` 已被 git 忽略；密钥只填在本地 `.env`，模板见 `.env.example`。
> - **KWS 模型不入库**（`models/` 已忽略，39MB）。换机器需手动把
>   `sherpa-onnx-kws-zipformer-zh-en-3M-2025-12-20` 拷到 `models/kws/` 下（缺失会自动降级，不影响启动）。
