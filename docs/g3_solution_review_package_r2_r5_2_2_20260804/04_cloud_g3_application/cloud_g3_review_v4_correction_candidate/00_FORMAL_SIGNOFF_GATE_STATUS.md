...
结论：`READY_FOR_SIGNOFF_CANDIDATE_PROMOTION`。V4 已部署到 10097，远端回归、
Bearer-only、命令主路径、关闭码、日志扫描和 REP-01 clean checkout 均通过。当前仍仅覆盖
测试阶段 G3 云侧 Web-only 技术整改候选包评审；如需复制为
`cloud_g3_review_v4_signoff_candidate`，不得扩大为生产上线、真实端侧联调完成或生产 TLS 验收。
...
| P1-02 clean 复现 | PASS | `e0aac93802afc017eaefc01adf5290ab7d44cdf9` clean checkout；`uv sync --all-extras --dev` PASS；126 tests PASS；Ruff PASS；脱敏扫描 0；`generated_artifacts=0`；post-run git status 为空 | 无 |
...
| P1-06 X3 actions | PASS | 4 个独立正向 case，纳入 clean checkout 126 tests 并通过 | 无 |
