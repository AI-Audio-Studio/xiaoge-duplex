...
| --- | --- | --- | --- |
| P0-01 API Key 泄漏事件 | 部分关闭 | 新 Key HTTP 200；缺失/错误 Key 均为 401；默认 key 已从 V4 Demo 删除；候选包扫描 0 命中 | 安全 owner 签署旧 Key 平台作废回执 |
| P0-01 历史 token | 技术验证 PASS、待 owner | token 仅驻留测试进程；610 秒后 Bearer 重连为 4401 | 部署/身份 owner 签署失效回执 |
...
| P1-01 源码可追溯 | PASS（技术证据） | 整改提交 `0710254d11d0ee84b0ab09e46d644fd283752461`，分支 `g3-cloud-v4-signoff-20260810`，完整 source snapshot 与 SHA-256 manifest | Release owner 确认可读取位置并签字 |
...
| P1-04 cmd lifecycle | 云侧本地已闭环 | unknown/duplicate/late/两类 timeout 测试 | 端云共同见证或正式 fake executor 记录 |
