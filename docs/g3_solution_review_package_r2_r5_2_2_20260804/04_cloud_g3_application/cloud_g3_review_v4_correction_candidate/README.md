...
- `P0-01`：新 Key 已在 10097 返回 HTTP 200，缺失/随机错误 Key 均返回 401；Security owner
  ...
  Identity/deployment owner 已确认历史 access token 失效。
- `P1-01`：整改源码已形成独立提交
...
- `G3_CLOUD_V4_JUDGE_REVIEW_RESPONSE_20260811.md`：对 2026-08-11 评委复审意见的逐项响应、补证计划和签收边界。
...
- `证据/签收/SRC-02_RELEASE_OWNER_SOURCE_CONFIRMATION_0710254d11d0_20260811.md`：Release owner 源码可追溯确认回执。
- `证据/签收/OWNER_WEB_ONLY_G3_CANDIDATE_SIGNOFF_20260811.md`：当前 Web-only/G3 candidate owner 签收记录。
- `证据/签收/SEC-01_API_KEY_CLOSURE_OWNER_DECLARATION_20260811.md`：旧 API Key 平台侧作废 owner 回执。
- `证据/签收/SEC-02B_TOKEN_INVALIDATION_OWNER_DECLARATION_20260811.md`：历史 access token 失效 owner 回执。
- `证据/签收/JOINT-01_WEB_ONLY_SCOPE_OWNER_DECLARATION_20260811.md`：当前 Web-only/G3 candidate 签收边界。
- `证据/签收/JOINT-01_END_CLOUD_SIGNOFF_10097_20260811.md`：测试阶段端云联合签收回执，不授权真实动作/生产上线。
