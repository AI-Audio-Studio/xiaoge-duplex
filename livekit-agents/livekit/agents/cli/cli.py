...
class ConsoleAudioOutput(io.AudioOutput):
    def __init__(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self._device_output_delay: float = 0.0
        self._device_buffer_duration: float = 0.0
    ...
    def update_output_timing(self, *, output_delay: float, buffer_duration: float) -> None:
        self._device_output_delay = max(output_delay, 0.0)
        self._device_buffer_duration = max(buffer_duration, 0.0)
    ...
    async def _wait_for_playout(self) -> None:
        async def _wait_buffered_audio() -> None:
            ...
            # Buffer empty only means we've handed the last block to PortAudio.
            # Wait for the device queue to drain so the tail isn't cut off.
            drain_wait = min(
                self._device_output_delay + self._device_buffer_duration + 0.02,
                0.5,
            )
            if drain_wait > 0:
        ...
        self._device_output_delay = 0.0
        self._device_buffer_duration = 0.0
...
class AgentsConsole:
    ...
    def _sd_output_callback(self, outdata: np.ndarray, frames: int, time: Any, *_: Any) -> None:
        ...
        self._io_audio_output.update_output_timing(
