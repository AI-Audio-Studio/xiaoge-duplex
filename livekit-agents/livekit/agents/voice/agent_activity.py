...
class AgentActivity(RecognitionHooks):
    ...
    def on_interruption(self, ev: inference.OverlappingSpeechEvent) -> None:
        logger.debug(
            "adaptive interruption accepted",
            extra={
                "event": "interruption_accepted",
                "source": "agent_activity.on_interruption",
                "probability": round(ev.probability, 3),
                "detection_delay": round(ev.detection_delay, 3),
                "total_duration": round(ev.total_duration, 3),
                "overlap_started_at": ev.overlap_started_at,
            },
        )
    ...
    def on_preemptive_generation(self, info: _PreemptiveGenerationInfo) -> None:
        ...
        logger.debug(
            ...
            extra={
                "event": "preemptive_generation_started",
                "source": "agent_activity.on_preemptive_generation",
                "retry_count": self._preemptive_generation_count,
                "transcript_len": len(info.new_transcript),
                "confidence": round(info.transcript_confidence, 3),
            },
        )
    ...
    def on_end_of_turn(self, info: _EndOfTurnInfo) -> bool:
        ...
        logger.debug(
            "end-of-turn callback received",
            extra={
                "event": "end_of_turn_callback",
                "source": "agent_activity.on_end_of_turn",
                "transcript_len": len(info.new_transcript),
                "confidence": round(info.transcript_confidence, 3),
                "skip_reply": info.skip_reply,
                "transcription_delay": (
                    round(info.transcription_delay, 3)
                    if info.transcription_delay is not None
                    else None
                ),
                "end_of_turn_delay": (
                    round(info.end_of_turn_delay, 3)
        ...
        if preemptive := self._preemptive_generation:
            ...
            if (
                ...
                logger.debug(
                    ...
                    extra={
                        "event": "preemptive_generation_reused",
                        "source": "agent_activity._user_turn_completed_task",
    ...
    async def _user_turn_exceeded_task(self, ev: UserTurnExceededEvent) -> None:
        ...
        logger.debug(
            ...
            extra={
                "event": "user_turn_limit_exceeded",
                "source": "agent_activity._user_turn_exceeded_task",
    ...
    async def _tts_task_impl(
        ...
        text_preview: str | None = text if isinstance(text, str) else None
        ...
        def _on_first_frame(fut: asyncio.Future[float] | asyncio.Future[None]) -> None:
            ...
            logger.debug(
                ...
                extra={
                    "event": "agent_playback_started",
                    "source": "agent_activity._tts_task_impl",
                    "speech_id": speech_handle.id,
                    "text_preview": _preview_text(text_preview),
                    "started_speaking_at": started_speaking_at,
                    "started_forwarding_at": started_forwarding_at,
                },
            )
        ...
        logger.debug(
            "agent playback completed",
            extra={
                "event": "agent_playback_completed",
                "source": "agent_activity._tts_task_impl",
                "speech_id": speech_handle.id,
                "text_preview": _preview_text(text_preview),
