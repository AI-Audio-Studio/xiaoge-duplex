...
class AudioRecognition:
    ...
    def on_start_of_speech(
        ...
        logger.debug(
            ...
            extra={
                ...
                "source": "audio_recognition.on_start_of_speech",
    ...
    def on_end_of_speech(
        ...
        logger.debug(
            ...
            extra={
                ...
                "source": "audio_recognition.on_end_of_speech",
                "ended_at": ended_at,
                "agent_speaking": self._agent_speaking,
                "interruption": interruption if is_given(interruption) else None,
                "turn_detection_mode": self._turn_detection_mode,
            },
        )
        if self._speaking:
    ...
    async def _on_stt_event(self, ev: stt.SpeechEvent) -> None:
        ...
        if ev.type == stt.SpeechEventType.FINAL_TRANSCRIPT:
            ...
            extra["event"] = "final_transcript_received"
            extra["source"] = "audio_recognition._on_stt_event"
            ...
            if self._vad_base_turn_detection or self._user_turn_committed:
                if transcript_changed:
                    logger.debug(
                        "preemptive generation candidate from final transcript",
                        extra={
                            "event": "preemptive_generation_candidate_final",
                            "source": "audio_recognition._on_stt_event",
                            "transcript_len": len(self._audio_transcript),
                            "confidence": round(
                                (
            ...
            logger.debug(
                ...
                extra={
                    "event": "preflight_transcript_received",
                    "source": "audio_recognition._on_stt_event",
            ...
            if self._turn_detection_mode != "manual" or self._user_turn_committed:
                ...
                logger.debug(
                    "preemptive generation candidate from preflight transcript",
                    extra={
                        "event": "preemptive_generation_candidate_preflight",
                        "source": "audio_recognition._on_stt_event",
                        "transcript_len": len(self._audio_preflight_transcript),
                        "confidence": round(sum(confidence_vals) / len(confidence_vals), 3),
                    },
                )
                self._hooks.on_preemptive_generation(
                    _PreemptiveGenerationInfo(
    ...
    def _run_eou_detection(self, chat_ctx: llm.ChatContext, skip_reply: bool = False) -> None:
        ...
        async def _bounce_eou_task(
            ...
            logger.debug(
                "end-of-turn decision computed",
                extra={
                    "event": "eou_decision_computed",
                    "source": "audio_recognition._run_eou_detection",
                    "turn_detector": type(turn_detector).__name__ if turn_detector else None,
                    "transcript_len": len(self._audio_transcript),
                    "language": str(self._last_language) if self._last_language else None,
                    "eou_probability": round(end_of_turn_probability, 3),
                    "unlikely_threshold": (
                        round(unlikely_threshold, 3) if unlikely_threshold is not None else None
                    ),
