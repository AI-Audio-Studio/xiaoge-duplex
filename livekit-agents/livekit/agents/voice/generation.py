...
async def _llm_inference_task(
    ...
    if isinstance(llm_node, str):
        ...
        logger.debug(
            "llm generation produced immediate text response",
            extra={
                "event": "llm_immediate_response",
                "source": "generation._llm_inference_task",
                "provider": provider,
                "model": model,
                "text_len": len(data.generated_text),
            },
        )
    ...
    logger.debug(
        ...
        extra={
            ...
            "provider": provider,
            "model": model,
            "ttft": round(data.ttft, 3) if data.ttft is not None else None,
            "text_len": len(data.generated_text),
            "function_calls": len(data.generated_functions),
        },
    )
    return True
...
async def _tts_inference_task(
    ...
    async def _tts_node_inference(input: AsyncIterable[str], pushed_duration: float) -> float:
        ...
        async for audio_frame in tts_node:
            if start_time is not None and data.ttfb is None:
                ...
                logger.debug(
                    "tts first audio frame received",
                    extra={
                        "event": "tts_first_audio_frame",
                        "source": "generation._tts_inference_task",
                        "provider": provider,
                        "model": model,
                        "ttfb": round(data.ttfb, 3),
                    },
                )
    ...
    logger.debug(
        ...
        extra={
            ...
            "provider": provider,
            "model": model,
            "ttfb": round(data.ttfb, 3) if data.ttfb is not None else None,
            "audio_generated": pushed_duration > 0,
            "audio_duration": round(pushed_duration, 3),
        },
    )
